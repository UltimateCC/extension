...
export default function OBS({ obsIsConnected, userConfig, updateConfig, resfreshObs, refreshing }: OBSProps) {
    ...
    const updateObsAuth = () => {
        ...
        if(!update) return setResponse({ type: "error", message: 'Invalid port' });
        ...
        <div className="obs">
            {userConfig.obsPassword && response && userConfig.obsEnabled && (
                <Alert
                    ...
                    reload={{runReload: resfreshObs, stopReloading: !refreshing}}
                />
            )}
