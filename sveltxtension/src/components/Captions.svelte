...
<script lang="ts">
	...
	export let captionsShown: boolean;
	...
	$: if($position.bottom === undefined && movableArea && movableElem) $position.bottom = 10;
	$: if($position.left === undefined && movableArea && movableElem) $position.left = 50 - movableElem.offsetWidth * 50 / movableArea.offsetWidth;
	...
	function getCaptionsStyle(settings: SettingsType) {
		...
		+ 'background-color:' + settings.backgroundColor + ';' //todo: hex to rgb   settings.backgroundOpacity
		//+ 'backdrop-filter:' + 'todo: blur'
	}
...
{#if captionsShown }
	<div id="caption-movable-area" bind:this={movableArea}>
		...
		<div id="caption-container"
			style={getCaptionsStyle($settings) + "left: "+$position.left +"%; bottom: "+$position.bottom +"%;" }
